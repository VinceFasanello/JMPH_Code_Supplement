...
myPCA <- principal(mydata[,c('tas_breadth', 'tas_position', 'pcp_breadth',
                             ...
                             'boundary_length', 'meanEle')], nfactors = 10, rotate = "none") # mean elevation added as discussed. 
myPCA$loadings

myPCA <- principal(mydata[,c('tas_breadth', 'tas_position', 'pcp_breadth',
                             'pcp_position', 'mtn_mass', 'wtr_mass',
                             'dispersal_ability', 'pair_age','distance', 'meanEle')], nfactors = 9, rotate = "none") # mean elevation added as discussed. 
myPCA$loadings
...
size = 2
alpha = 0.7
# PC1 decreases with latitude (relationship is weakest in Afrotropics)
x1 <- ggplot(world)+ geom_sf() + geom_point(data = mydata[order(mydata$PC1, decreasing = F),], aes(y=lat, x=lon, color = PC1), alpha = alpha, size = size)+ 
...
# x4 <- ggplot(world)+ geom_sf() + geom_point(data = mydata[order(mydata$PC4, decreasing = F),], aes(y=lat, x=lon, color = PC4), alpha = 0.8) + 
