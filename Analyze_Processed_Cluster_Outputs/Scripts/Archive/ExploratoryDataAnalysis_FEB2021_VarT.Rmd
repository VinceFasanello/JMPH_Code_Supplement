...
moran.test(residuals(m), nb2listw(nb))$p.value # weak evidence for spatial component. 
...
# vector 1 added ---
m <- gls(lcost ~ lat +  I(lat^2) + V17 + V18 + V19 + V20, correlation = corVince(0.99, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); cor(predict(m),log(mydata$lcost))
matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
...
m <- gls(scale(lcost) ~ scale(pair_age) + scale(range_size) + scale(niche_position) + scale(niche_breadth) + scale(niche_overlap)  + scale(distance) + scale(mtnmass),
         correlation = corPagel(0.989, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); car::vif(m); cor(predict(m),log(mydata$lcost)) # remove mtn mass
m <- gls(scale(lcost) ~ scale(pair_age) + scale(range_size) + scale(niche_position) + scale(niche_breadth) + scale(niche_overlap)  + scale(distance),
         correlation = corPagel(0.989, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); car::vif(m); cor(predict(m),log(mydata$lcost)) # remove pair age (does not become significant with spatial filtering)
...
# spatial filtering --------------------
sarcol <- SpatialFiltering(formula = scale(lcost) ~ scale(range_size) + scale(niche_position) + scale(niche_breadth) + scale(niche_overlap)  + scale(distance),
...
m <- gls(scale(lcost) ~ scale(range_size) + scale(niche_position) + scale(niche_breadth) + scale(niche_overlap)  + scale(distance) + V19 + V20,
