...
mylambda <-  0.4534931  # m$modelStruct$corStruct[1]
...
#          correlation = corPagel(0.99, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); car::vif(m) # remove range size
m <- gls(scale(lcost) ~  scale(niche_position) + scale(niche_breadth) + scale(niche_overlap) + scale(distance),
...
# # spatial filtering --------------------
# sarcol <- SpatialFiltering(formula = scale(lcost) ~ scale(niche_position) + scale(niche_breadth) + scale(niche_overlap) + scale(distance), data = mydata,nb=nb, style="W", ExactEV = TRUE)
# mydata[,c((basecols+1):(basecols+1+ dim(fitted(sarcol))[2]-1))]<-fitted(sarcol)
# 
# # vector 1 added ---
# m <- gls(scale(lcost) ~ scale(niche_position) + scale(niche_breadth) + scale(niche_overlap) + scale(distance) + V16, correlation = corPagel(mylambda, phy = tree, fixed = T), data = mydata, method = "REML"); summary(m)
# matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
# spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
# moran.test(residuals(m), nb2listw(nb))$p.value # Too much!
