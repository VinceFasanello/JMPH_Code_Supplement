...
# model reduction ----------------------
m <- gls(lcost ~ lat +  I(lat^2), correlation = corPagel(0.989, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m)
...
# vector 1 added ---
m <- gls(lcost ~ lat + V17, correlation = corVince(0.99, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); cor(predict(m),log(mydata$lcost))
matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
moran.test(residuals(m), nb2listw(nb))$p.value # too much.

# # vector 2 added --- 
m <- gls(lcost ~ lat + V17 + V18, correlation = corVince(0.99, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); cor(predict(m),log(mydata$lcost))
matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
moran.test(residuals(m), nb2listw(nb))$p.value # too much.
...
# plot ---------------------------------
m <- gls(lcost ~ lat, correlation = corVince(0.99, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); cor(predict(m),log(mydata$lcost))
...
m <- gls(scale(lcost) ~ scale(pair_age) + scale(range_size) + scale(niche_position) + scale(niche_breadth)  + scale(distance) + scale(mtnmass) + scale(wtrmass) + scale(lat),
...
m <- gls(scale(lcost) ~ scale(pair_age) + scale(range_size) + scale(niche_position) + scale(niche_breadth)  + scale(distance) + scale(wtrmass) + scale(lat),
...
m <- gls(scale(lcost) ~ scale(range_size) + scale(niche_position) + scale(niche_breadth)  + scale(distance) + scale(mtnmass) + scale(wtrmass) + scale(lat),
         correlation = corPagel(0.989, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); car::vif(m); cor(predict(m),log(mydata$lcost)) # wtr mass
m <- gls(scale(lcost) ~ scale(range_size) + scale(niche_position) + scale(niche_breadth)  + scale(distance) + scale(mtnmass) + scale(lat),
...
m <- gls(scale(lcost) ~ scale(pair_age) + scale(range_size) + scale(niche_position) + scale(niche_breadth)  + scale(distance) + scale(mtnmass) + scale(wtrmass) + scale(abs(lat)),
         correlation = corPagel(0.989, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); car::vif(m); cor(predict(m),log(mydata$lcost)) # abslat
m <- gls(scale(lcost) ~ scale(pair_age) + scale(range_size) + scale(niche_position) + scale(niche_breadth)  + scale(distance) + scale(mtnmass) + scale(wtrmass),
         correlation = corPagel(0.989, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); car::vif(m); cor(predict(m),log(mydata$lcost)) # pairage
