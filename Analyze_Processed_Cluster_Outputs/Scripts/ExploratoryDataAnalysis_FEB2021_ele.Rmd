...
mylambda <-  0.1641264 # m$modelStruct$corStruct[1]
m <- gls(lcost ~ lat, correlation = corPagel(mylambda, phy = tree, fixed = T), data = mydata, method = "REML")
...
# vector 1 added --- 
m <- gls(lcost ~ lat + V16, correlation = corPagel(mylambda, phy = tree, fixed = T), data = mydata, method = "REML"); summary(m) 
matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
moran.test(residuals(m), nb2listw(nb))$p.value # OK! keep going....

# vector 2 added --- 
m <- gls(lcost ~ lat + V16 + V17, corPagel(mylambda, phy = tree, fixed = T), data = mydata, method = "REML"); summary(m) 
matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
moran.test(residuals(m), nb2listw(nb))$p.value # OK! keep going....

# vector 3 added --- 
m <- gls(lcost ~ lat + V16 + V17 + V18, corPagel(mylambda, phy = tree, fixed = T), data = mydata, method = "REML"); summary(m) 
matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
moran.test(residuals(m), nb2listw(nb))$p.value # OK! keep going....

# vector 4 added --- 
m <- gls(lcost ~ lat + V16 + V17 + V18 + V19, corPagel(mylambda, phy = tree, fixed = T), data = mydata, method = "REML"); summary(m) 
matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
moran.test(residuals(m), nb2listw(nb))$p.value #  OK! keep going....

# vector 5 added --- 
m <- gls(lcost ~ lat + V16 + V17 + V18 + V19 + V20, corPagel(mylambda, phy = tree, fixed = T), data = mydata, method = "REML"); summary(m) 
matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
moran.test(residuals(m), nb2listw(nb))$p.value #  OK! keep going....

# vector 6 added --- 
m <- gls(lcost ~ lat + V16 + V17 + V18 + V19 + V20 + V21, corPagel(mylambda, phy = tree, fixed = T), data = mydata, method = "REML"); summary(m) 
matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
moran.test(residuals(m), nb2listw(nb))$p.value # OK.

# # vector 7 added --- 
# m <- gls(lcost ~ lat + V16 + V17 + V18 + V19 + V20 + V21 + V22, corPagel(mylambda, phy = tree, fixed = T), data = mydata, method = "REML"); summary(m) 
# matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
# spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
...
m <- gls(scale(lcost) ~ scale(range_size) + scale(niche_position) + scale(niche_breadth) + scale(niche_overlap) + scale(distance),
         correlation = corPagel(0.99, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); car::vif(m) # OK.
mylambda <- -0.01123826
...
# vector 1 added --- 
m <- gls(scale(lcost) ~ scale(range_size) + scale(niche_position) + scale(niche_breadth) + scale(niche_overlap) + scale(distance) + V16, correlation = corVince(mylambda, phy = tree, fixed = T), data = mydata, method = "REML"); summary(m) 
matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
moran.test(residuals(m), nb2listw(nb))$p.value # OK! keep going....
