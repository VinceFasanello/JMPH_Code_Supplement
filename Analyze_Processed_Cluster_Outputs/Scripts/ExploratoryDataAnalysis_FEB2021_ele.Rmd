...
mydata$niche_breadth <- mydata$MAT_range_pair_mean # mydata[,paste0(costvar, "_range_pair_mean")]
...
# mydata$Species.1bl[is.na(mydata$realm)] # [1] "Cinclocerthia ruficauda" "Myadestes genibarbis"    "Agelaius humeralis"      "Edolisoma ostentum"      "Lalage leucopyga" 
# mydata$realm[is.na(mydata$realm)] <- c("new", "new", "new", "old", "old")
# # ----------------
...
CB models
```{r}
mydata$thermal_niche_breadth <- mydata$niche_breadth
mydata <- mydata[,which(colnames(mydata) != "niche_breadth")]

# elevational barrier size related to thermal niche breadth??? YES
...
m <- gls(lcost ~ lat, correlation = corPagel(0.989, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); cor(predict(m),log(mydata$lcost))
hist(resid(m), breaks = 40)

# spatial autocorr ---------------------
matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
moran.test(residuals(m), nb2listw(nb))$p.value # no evidence for spatial autocorrelation, but lets add all the possible terms anyways to see if the model is significantly changed!
...
colnames(mydata) # 6 vectors created

# vector 1 added ---
m <- gls(lcost ~ lat + V17 + V18 + V19 + V20 + V21 + V22, correlation = corVince(0.999, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); cor(predict(m),log(mydata$lcost))
matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
...
# plot ---------------------------------
m <- gls(lcost ~ lat, correlation = corVince(0.989, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); cor(predict(m),log(mydata$lcost))
...
```{r}
# m <- gls(scale(lcost) ~ scale(pair_age) + scale(range_size) + scale(niche_position) + scale(niche_breadth) + scale(niche_overlap)  + scale(distance) + scale(mtnmass) + scale(wtrmass) + scale(lat) + scale(I(lat^2)),
#          correlation = corPagel(0.99, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); car::vif(m); cor(predict(m),log(mydata$lcost)) # pair age
# m <- gls(scale(lcost) ~ scale(range_size) + scale(niche_position) + scale(niche_breadth) + scale(niche_overlap)  + scale(distance) + scale(mtnmass) + scale(wtrmass) + scale(lat) + scale(I(lat^2)),
...
# m <- gls(scale(lcost) ~ scale(range_size) + scale(niche_position) + scale(niche_breadth) + scale(niche_overlap)  + scale(distance) + scale(mtnmass) + scale(lat),
#          correlation = corPagel(0.99, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); car::vif(m); cor(predict(m),log(mydata$lcost)) # lat (LAT DOES NOT BECOME SIGNIFICANT WIHT SPATIAL FILTERING BUT IS STILL RIGHT ON THE BORDER!!!> >>> INCluding it increases AIC/BIC....so omit.)
...
# 
# # vector 1 added ---
# m <- gls(scale(lcost) ~ scale(range_size) + scale(niche_position) + scale(niche_breadth) + scale(niche_overlap)  + scale(distance) + scale(mtnmass) + V19 + V20 + V21 + V22, correlation = corVince(0.99, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); cor(predict(m),log(mydata$lcost))
# matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
# spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
# moran.test(residuals(m), nb2listw(nb))$p.value # too much.
# 
...
m <- gls(scale(lcost) ~ scale(range_size) + scale(niche_position) + scale(niche_breadth) + scale(distance) + scale(mtnmass),
         correlation = corPagel(0.989, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); car::vif(m); cor(predict(m),log(mydata$lcost)) 
# DONE. Model is identical to above. 
```
...
# spatial filtering --------------------
sarcol <- SpatialFiltering(formula = scale(lcost) ~ scale(range_size) + scale(niche_position) + scale(niche_breadth) + scale(distance) + scale(mtnmass),
                           data = mydata,nb=nb, style="W", ExactEV = TRUE)
mydata[,c((basecols+1):(basecols+1+ dim(fitted(sarcol))[2]-1))]<-fitted(sarcol)
colnames(mydata) # 2 vectors created. 

# add spatial filtering vectors to model with lambda=0.99
m <- gls(scale(lcost) ~ scale(range_size) + scale(niche_position) + scale(niche_breadth) + scale(distance) + scale(mtnmass) + V19,
...
m <- gls(scale(lcost) ~ scale(range_size) + scale(niche_position) + scale(niche_breadth) + scale(distance) + scale(mtnmass) + V19 + V20,
