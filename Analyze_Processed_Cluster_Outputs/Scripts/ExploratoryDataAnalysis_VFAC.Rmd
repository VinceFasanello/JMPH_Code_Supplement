...
```{r}
mycost <- "cmean_btm25" # OPTIONS: "cmin", "cmean", "cmean_btm50, cmean_btm25, "cmedian
...
mymodq <- gls(lcost ~ splatmean + I(splatmean^2) + 
                I(rowMeans(cbind(mean_ele_sp1, mean_ele_sp2))) + 
                I(rowMeans(cbind(ele_range_sp1, ele_range_sp2))) + 
                centroid_distance + 
                Pair.age..MY.,
              correlation = corPagel(1, phy = ELEpruned.tree), data = ELEmydata, method = "REML")
# mymodq <- glm(lcost ~ splatmean + I(splatmean^2) + I(rowMeans(cbind(mean_ele_sp1, mean_ele_sp2))) +  I(rowMeans(cbind(ele_range_sp1, ele_range_sp2))) + centroid_distance + Pair.age..MY., data = ELEmydata)
summary(mymodq)
coefficients(mymodq)
...
y <- ELEmydata$lcost
x <- ELEmydata[,c("splatmean", "splatmean2")]
