...
mydata$Species.1bl[is.na(mydata$realm)] # [1] "Myadestes genibarbis"   "Lalage leucopyga"  
mydata$realm[is.na(mydata$realm)] <- c("new", "old")
# ----------------
...
# vector 1 added --- 
m <- gls(lcost ~ lat+  I(lat^2) + V16, correlation = corPagel(mylambda, phy = tree, fixed = T), data = mydata, method = "REML"); summary(m) 
matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
moran.test(residuals(m), nb2listw(nb))$p.value # OK! keep going....

# vector 2 added ---
m <- gls(lcost ~ lat +  I(lat^2) + V16 + V17, corPagel(mylambda, phy = tree, fixed = T), data = mydata, method = "REML"); summary(m)
matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
moran.test(residuals(m), nb2listw(nb))$p.value # OK! keep going....

# vector 3 added ---
m <- gls(lcost ~ lat +  I(lat^2) + V16 + V17 + V18, corPagel(mylambda, phy = tree, fixed = T), data = mydata, method = "REML"); summary(m)
matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
moran.test(residuals(m), nb2listw(nb))$p.value # OK! keep going....

# vector 4 added ---
m <- gls(lcost ~ lat +  I(lat^2) + V16 + V17 + V18 + V19, corPagel(mylambda, phy = tree, fixed = T), data = mydata, method = "REML"); summary(m)
matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
moran.test(residuals(m), nb2listw(nb))$p.value # OK! keep going....

# vector 5 added ---
m <- gls(lcost ~ lat +  I(lat^2) + V16 + V17 + V18 + V19 + V20, corPagel(mylambda, phy = tree, fixed = T), data = mydata, method = "REML"); summary(m)
matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
moran.test(residuals(m), nb2listw(nb))$p.value # Too much!

# vector 6 added ---
m <- gls(lcost ~ lat +  I(lat^2) + V16 + V17 + V18 + V19 + V20 + V21, corPagel(mylambda, phy = tree, fixed = T), data = mydata, method = "REML"); summary(m)
matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
moran.test(residuals(m), nb2listw(nb))$p.value # Too much!

# vector 7 added ---
m <- gls(lcost ~ lat +  I(lat^2) + V16 + V17 + V18 + V19 + V20 + V21 + V22, corPagel(mylambda, phy = tree, fixed = T), data = mydata, method = "REML"); summary(m)
matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
moran.test(residuals(m), nb2listw(nb))$p.value # Too much!

# vector 8 added ---
m <- gls(lcost ~ lat +  I(lat^2) + V16 + V17 + V18 + V19 + V20 + V21 + V22 + V23, corPagel(mylambda, phy = tree, fixed = T), data = mydata, method = "REML"); summary(m); cor(predict(m),log(mydata$lcost))
matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
moran.test(residuals(m), nb2listw(nb))$p.value # Too much!
...
ggplot(world)+
  geom_sf() + 
  geom_point(data = mydata, aes(y=lat, x=lon, color = V18)) + 
  scale_color_viridis()
...
# view spatial component 5 ---
ggplot(world)+
  geom_sf() + 
  geom_point(data = mydata, aes(y=lat, x=lon, color = V20)) + 
  scale_color_viridis()

# view spatial component 6 ---
ggplot(world)+
  geom_sf() + 
  geom_point(data = mydata, aes(y=lat, x=lon, color = V21)) + 
  scale_color_viridis()

# view spatial component 7 ---
ggplot(world)+
  geom_sf() + 
  geom_point(data = mydata, aes(y=lat, x=lon, color = V22)) + 
  scale_color_viridis()

# view spatial component 8 ---
ggplot(world)+
  geom_sf() + 
  geom_point(data = mydata, aes(y=lat, x=lon, color = V23)) + 
  scale_color_viridis()
...
# view a ternary plot ---- this kills your ability to plot and you need to produce subsequent plots ... requires reload. 
# require(ggtern)
# ggtern(data=mydata,aes(wtrmass,mtnmass,I(1-(mtnmass+wtrmass)), color = lcost)) +
#     stat_confidence_tern(breaks=seq(0,0.9,0.1), color = "gray")+
#     geom_point() + 
#     scale_color_viridis()+
#    theme_showarrows() +  theme_rgbw() +  theme_hidetitles() 
...
# lat & Lat2
m <- gls(scale(lcost) ~ scale(pair_age) + scale(range_size) + scale(niche_position) + scale(niche_breadth)  + scale(distance) + scale(mtnmass) + scale(wtrmass) + scale(lat) + scale(lat2),
         correlation = corPagel(0.99, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); car::vif(m); cor(predict(m),log(mydata$lcost)) #wtrmass
m <- gls(scale(lcost) ~ scale(pair_age) + scale(range_size) + scale(niche_position) + scale(niche_breadth)  + scale(distance) + scale(mtnmass) + scale(lat) + scale(lat2),
...
m <- gls(scale(lcost) ~ scale(pair_age) + scale(range_size) + scale(niche_position) + scale(niche_breadth)  + scale(distance) + scale(mtnmass) + scale(lat),
         correlation = corPagel(0.989, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); car::vif(m); cor(predict(m),log(mydata$lcost)) #mtnmass
m <- gls(scale(lcost) ~ scale(pair_age) + scale(range_size) + scale(niche_position) + scale(niche_breadth)  + scale(distance) + scale(lat),
         correlation = corPagel(0.989, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); car::vif(m); cor(predict(m),log(mydata$lcost)) #age
m <- gls(scale(lcost) ~ scale(range_size) + scale(niche_position) + scale(niche_breadth)  + scale(distance) + scale(lat),
...
# problem is that latitude is not well described by a linear term so this makes little sense...try with abslat instead
plot(mydata$lcost~mydata$lat)
...
m <- gls(scale(lcost) ~ scale(pair_age) + scale(range_size) + scale(niche_position) + scale(niche_breadth)  + scale(distance) + scale(mtnmass) + scale(wtrmass) + scale(abs(lat)),
         correlation = corPagel(0.989, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); car::vif(m); cor(predict(m),log(mydata$lcost)) # watermass
m <- gls(scale(lcost) ~ scale(pair_age) + scale(range_size) + scale(niche_position) + scale(niche_breadth)  + scale(distance) + scale(mtnmass) + scale(abs(lat)),
         correlation = corPagel(0.989, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); car::vif(m); cor(predict(m),log(mydata$lcost)) # mtn mass
m <- gls(scale(lcost) ~ scale(pair_age) + scale(range_size) + scale(niche_position) + scale(niche_breadth)  + scale(distance) + scale(abs(lat)),
         correlation = corPagel(0.989, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); car::vif(m); cor(predict(m),log(mydata$lcost)) # age
m <- gls(scale(lcost) ~scale(range_size) + scale(niche_position) + scale(niche_breadth)  + scale(distance) + scale(abs(lat)),
         correlation = corPagel(0.989, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); car::vif(m); cor(predict(m),log(mydata$lcost)) # position
m <- gls(scale(lcost) ~scale(range_size) + scale(niche_breadth)  + scale(distance) + scale(abs(lat)),
         correlation = corPagel(0.989, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); car::vif(m); cor(predict(m),log(mydata$lcost))
hist(resid(m), breaks = 25) # nice looking residuals!
plot(mydata$lcost~abs(mydata$lat))

# DONE. Model is similar to above -- niche position and abs lat were conflated in a way that lat and niche position were not!.
# this second model has a better fit (predictive power & ACI/BIC/logLik)
```
...
m <- gls(scale(lcost) ~ scale(pair_age) + scale(range_size) + scale(niche_position) + scale(niche_breadth) + scale(distance) + scale(mtnmass) + scale(wtrmass),
         correlation = corPagel(0.989, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); car::vif(m); cor(predict(m),log(mydata$lcost)) # wtr mass
m <- gls(scale(lcost) ~ scale(pair_age) + scale(range_size) + scale(niche_position) + scale(niche_breadth) + scale(distance) + scale(mtnmass),
         correlation = corPagel(0.989, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); car::vif(m); cor(predict(m),log(mydata$lcost)) # mtn mass
m <- gls(scale(lcost) ~ scale(pair_age) + scale(range_size) + scale(niche_position) + scale(niche_breadth) + scale(distance),
         correlation = corPagel(0.989, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); car::vif(m); cor(predict(m),log(mydata$lcost)) # age
m <- gls(scale(lcost) ~  scale(range_size) + scale(niche_position) + scale(niche_breadth) + scale(distance),
         correlation = corPagel(0.989, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m); car::vif(m); cor(predict(m),log(mydata$lcost))
# DONE. 
```
...
# spatial filtering --------------------
sarcol <- SpatialFiltering(formula = scale(lcost) ~  scale(range_size) + scale(niche_position) + scale(niche_breadth) + scale(distance),
...
m <- gls(scale(lcost) ~  scale(range_size) + scale(niche_position) + scale(niche_breadth) + scale(distance) + V19,
         correlation = corPagel(0.989, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m);cor(predict(m),log(mydata$lcost)) # vector 1
matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
moran.test(residuals(m), nb2listw(nb))$p.value # OK! keep going....

m <- gls(scale(lcost) ~  scale(range_size) + scale(niche_position) + scale(niche_breadth) + scale(distance) + V19 + V20,
...
m <- gls(scale(lcost) ~  scale(range_size) + scale(niche_position) + scale(niche_breadth) + scale(distance) + V19 + V20 + V21,
         correlation = corPagel(0.989, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m);cor(predict(m),log(mydata$lcost)) # vector 2
matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
moran.test(residuals(m), nb2listw(nb))$p.value # OK!
...
m <- gls(scale(lcost) ~  scale(range_size) + scale(niche_position) + scale(niche_breadth) + scale(distance) + V19 + V20 + V21 + V22,
         correlation = corPagel(0.989, phy = tree, fixed = F), data = mydata, method = "REML"); summary(m);cor(predict(m),log(mydata$lcost)) # vector 2
matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
moran.test(residuals(m), nb2listw(nb))$p.value # OK!
