...
mylambda <-  0.4978097  # m$modelStruct$corStruct[1]
m <- gls(lcost ~  lat +  I(lat^2), correlation = corPagel(mylambda, phy = tree, fixed = T), data = mydata, method = "REML"); summary(m)
...
mylambda <- 0.08256176
m <- gls(scale(lcost) ~ scale(range_size) + scale(niche_position) + scale(niche_breadth) + scale(niche_overlap) + scale(distance),
         correlation = corPagel(mylambda, phy = tree, fixed = T), data = mydata, method = "REML"); summary(m); car::vif(m)
...
# vector 1 added ---
m <- gls(scale(lcost) ~ scale(range_size) + scale(niche_position) + scale(niche_breadth) + scale(niche_overlap) + scale(distance) + V16, correlation = corPagel(mylambda, phy = tree, fixed = T), data = mydata, method = "REML"); summary(m)
matx <- as.matrix(m$residuals); rownames(matx) <- rownames(mydata)
spac <- lets.correl(x=matx, y=distm, z=12, equidistant = T, plot = T)
moran.test(residuals(m), nb2listw(nb))$p.value  # OK! keep going....
