...
# load rasters -----------------------------------------------------------------
# Elevation
setwd(wdtopo); load(file = "Elev_raster.rdata"); crs(Elev_raster)
# Temperature
setwd(wdclimate); load(file = "tasmax_rasters_VJF.rdata"); load(file = "tasmin_rasters_VJF.rdata"); load(file = "tasrng_rasters_VJF.rdata"); crs(tasmax)
# Precipitation
setwd(wdclimate); load(file = "pcpmax_rasters_VJF.rdata"); load(file = "pcpmin_rasters_VJF.rdata"); load(file = "pcprng_rasters_VJF.rdata"); crs(pcpmax)
...
# load(file = paste0(batchNumber, "_Main_Batch_Results_origin_",originStart,"_to_",originEnd,".rdata")) # uncomment for hot start!
...
for (i in sp_start:nrow(cooneyp)){
  ...
  # checkpoint. save every five species in case of a crash. -----
  if(i %% 5 == 0){
    completed_rows <- i - 1
