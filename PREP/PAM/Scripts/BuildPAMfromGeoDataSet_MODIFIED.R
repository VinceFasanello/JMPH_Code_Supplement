...
BuildPAMfromGeoDataSet <- function(fc, specieslabel, presencelabel, seasonallabel, PAMfname, myEnvRasters, is.bird = F) {
  ...
  for (i in 1:length(ALLSPP)) {
    ...
    if (myeval) {
      if (!is.na(presencelabel))  {
        thisPAM <- lets.presab(mysp, crs.grid = CRS('+proj=longlat +datum=WGS84'), 
                               xmn = -180, xmx = 180,
                               ymn = -90, ymx = 90,
                               resol = 0.5, remove.cells = F,
        ...
        thisPAM <- lets.presab(mysp, crs.grid = CRS('+proj=longlat +datum=WGS84'), 
                               xmn = -180, xmx = 180,
                               ymn = -90, ymx = 90,
                               resol = 0.5, remove.cells = F, count = F, remove.sp = F)
      }
  ...
  myPAM$Presence_and_Absence_Matrix <- getBands(myPAM$Presence_and_Absence_Matrix, c(-90,90), "AllSPPinClade")
...
require(letsR)
require(raster)
memory.size(max = 100000)
setwd("C:/Users/VJF/Desktop/Rebuild_PAMs"); load("BOTEROLAB.06Jan2019.ClimateRasters.1850.2005.CCSM4.WaterAndLand.RData")
load("TopoRasters_LandAndWater.Rdata")
...
myEnvRasters <-  projectRaster(myEnvRasters, crs = CRS('+proj=longlat +datum=WGS84'))
...
# Birds
setwd("C:/Users/VJF/Desktop/Rebuild_PAMs")
dsn = "BOTW.gdb"
layer = 'All_Species'
fc <- st_read(dsn = dsn, layer = layer)

specieslabel = 'SCINAME'
PAMfname = 'BirdPAM.Rdata'
