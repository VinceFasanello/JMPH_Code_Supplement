...
# directory paths --------------------------------------------------------------
wdin <- "C:/Users/VJF/Desktop/passback/ClusterFiles_MAT/to_scratch_inputs_mat"
wdout <- "C:/Users/VJF/Desktop/passback"
...
MAT_raster <- MAT; rm(MAT)
MAT_raster[values(Elev_raster) <= 0] <- NA; rm(Elev_raster)
...
setwd(wdout)
# load(file = paste0(batchNumber, "_Main_Batch_Results_origin_",originStart,"_to_",originEnd,"_MAT.rdata")) # willy only exist if there is a hotstart
...
for (i in sp_start:nrow(cooneyp)){
  ...
  crs(destinations) <- crs(MAT_raster)
  # full path savelists ----------------
  ...
  for (j in originStart:originEnd){
    ...
    if (!is.na(m_a_ids_block[i,j])){
      ...
      # create cost map ----------
      cost_map <- MAT_raster
      values(cost_map) <- abs(values(cost_map) - m_a_MAT_block[i,j])
      ...
      if(sum(!is.na(mypaths$mcost)) > 0){
        ...
        if (!is.na(mypath_id)){
          savelist_full_paths_mat <- c(savelist_full_paths_mat, mypaths$sPath[mypath_id])
          ...
          m_aB_MAT_mcosts_block[i,j] <- mypaths$mcost[mypath_id]
          m_aB_MAT_plengths_block[i,j] <- mypaths$plen[mypath_id]
          destination <- destinations[mypath_id,]
          m_a_bblons_MAT_block[i,j] <- destination$lon
          m_a_bblats_MAT_block[i,j] <- destination$lat
        ...
        } else {savelist_full_paths_mat <- c(savelist_full_paths_mat, NA)}
  ...
  save(savelist_full_paths_mat, file = paste0(batchNumber, "_MAT_LCP_Paths_for_", cooneyp[i,"Species.1bl"], "_", cooneyp[i,"Species.2bl"],"_origin_",originStart,"_to_",originEnd, ".rdata"))
  completed_rows <- i
  save(completed_rows, 
       m_a_bblons_MAT_block, m_a_bblats_MAT_block, m_aB_MAT_mcosts_block, m_aB_MAT_plengths_block,
       file = paste0(batchNumber, "_Main_Batch_Results_origin_",originStart,"_to_",originEnd,"_MAT.rdata"))
