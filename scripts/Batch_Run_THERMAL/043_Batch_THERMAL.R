...
for (i in sp_start:sp_end){
  ...
  for (j in 1:nrow(origins)) {
    ...
    # plot_map <- cost_map
    # plot_map[1] <- 0; plot_map[2] <- 147.413159102 # populate the first two cells with a minimum and maximum cost value to allow easy colorscaling among plots.
    # plot_map <- log(plot_map+1) # fix the distribution of costs so that variation in the data are visable. 
    # plot(plot_map, col = terrain.colors(255)); points(ai); lines(mypaths$sPath)
