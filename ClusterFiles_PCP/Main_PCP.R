# this script is painfully close, but will not work until I go back and do some fixing on PRECIP stuff...
# I want to do a simpler metric of precip than was originally used......will take some coding to address this. 
...
# directory paths --------------------------------------------------------------
wdin <- "/scratch/cbotero/inputs_pcp"
wdin <-"~/Box Sync/CB_VF_Shared/Dry_Lab/Projects/JMPH/ClusterFiles_PCP/to_scratch_inputs_mat"
wdout <- "/scratch/cbotero/outputs_pcp"
wdout <- "~/Box Sync/CB_VF_Shared/Dry_Lab/Projects/JMPH/ClusterFiles_PCP/outputs_pcp"
...
PCP_raster <- PCP; rm(PCP)
PCP_raster[values(Elev_raster) <= 0] <- NA; rm(Elev_raster)
...
setwd(wdout)
# load(file = paste0(batchNumber, "_Main_Batch_Results_origin_",originStart,"_to_",originEnd,"_PCP.rdata")) # willy only exist if there is a hotstart
...
for (i in sp_start:nrow(cooneyp)){
  ...
  crs(destinations) <- crs(PCP_raster)
  # full path savelists ----------------
  ...
  for (j in originStart:originEnd){
    ...
    if (!is.na(m_a_ids_block[i,j])){
      ...
      # create cost map ----------
      cost_map <- PCP_raster
      values(cost_map) <- abs(values(cost_map) - m_a_pcp_block[i,j])
      ...
      if(sum(!is.na(mypaths$mcost)) > 0){
        ...
        for (k in 1:length(mypaths$sPath)) {
          print(paste0("starting species ", i, " origin ", j, " destination ", k, " PCP"))
        ...
        } else {savelist_full_paths_pcp <- c(savelist_full_paths_pcp, NA)}
  ...
  save(savelist_full_paths_pcp, file = paste0(batchNumber, "_PCP_LCP_Paths_for_", cooneyp[i,"Species.1bl"], "_", cooneyp[i,"Species.2bl"],"_origin_",originStart,"_to_",originEnd, ".rdata"))
  completed_rows <- i
  save(completed_rows, 
       m_a_bblons_pcp_block, m_a_bblats_pcp_block, m_aB_pcp_mcosts_block, m_aB_pcp_plengths_block,
       file = paste0(batchNumber, "_Main_Batch_Results_origin_",originStart,"_to_",originEnd,"_PCP.rdata"))
