...
# directory paths --------------------------------------------------------------
wdin <- "~/Box Sync/JMPH/ThinBatch1Test/Inputs"
wdout <- "~/Box Sync/JMPH/ThinBatch1Test/Outputs_P"
...
setwd(wdout)
load(file = paste0(batchNumber, "_Main_Batch_Results_origin_",originStart,"_to_",originEnd,"_P.rdata")) # willy only exist if there is a hotstart
...
for (i in sp_start:nrow(cooneyp)){
    ...
    if (!is.na(m_a_ids_block[i,j])){
      ...
      if(!is.na(mypaths$plen[1])){
        ...
        for (k in 1:length(mypaths$sPath)) {
          print(paste0("starting species ", i, " origin ", j, " destination ", k, " PRECIPITATION"))
  ...
  save(completed_rows, 
       ...
       file = paste0(batchNumber, "_Main_Batch_Results_origin_",originStart,"_to_",originEnd,"_P.rdata"))
}
